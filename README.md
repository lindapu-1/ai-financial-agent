...
- **Canvas 文件上传优化**：
  - 支持上传 PDF、DOCX、MD 等格式文件，自动提取文本内容
  - 实现可视化文件卡片 UI，文件以卡片形式显示在编辑区，而非直接显示文本内容
  - 文件内容存储在隐藏数据中，仅在 AI 生成时作为上下文使用
  - 优化文件删除功能，支持删除单个文件卡片
- **Canvas 编辑器优化**：
  - 修复文件下方输入框无法输入的问题
  - 优化项目切换时的内容刷新速度，使用 `key` prop 强制重新渲染
  - 添加 `useEffect` 监听项目变化，确保内容立即同步
- **Canvas Chat 优化**：
  - 实现自动滚动到底部功能，发送消息后自动跳转到最新消息
  - 修复刷新页面后回到顶部的问题
- **错误处理与超时机制**：
  - 添加 5 秒超时检测，无响应时自动关闭加载状态并显示错误提示
  - 优化停止按钮功能，确保点击后立即清理所有加载状态
  - 添加详细的日志记录，包括 Prompt 长度分析、流式响应状态追踪
- **Bug 修复**：
  - ✅ 修复 Canvas 数据不显示在 General 数据库的问题（已在 pu 1.18 中完成）
  - ✅ 修复 React 无限更新循环问题（Maximum update depth exceeded）
  - ✅ 修复 Canvas 模式下有时收不到 AI 回复的问题
  - ✅ 修复文件卡片删除不生效的问题
