...
4.  **[Step 4] 独立 AI 逻辑实现 (In Progress 🏗️)**：
    *   **Context 注入**：将编辑器内容实时作为 AI 对话的上下文。
    *   **Skill 封装**：实现 "行业情况"、"技术壁垒" 等 Skills 的 Prompt 模板。
    *   **API 分流**：重构 `/api/chat` 以支持 `canvas` 模式下的独立逻辑流。
5.  **后续拓展**：一键填空、自定义 Skill、多格式导出。
